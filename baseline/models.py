...
class Discriminator(nn.Module):
    def __init__(self):
        ...
        self.surrogate_encoder = nn.Sequential(
            new_block(in_channels=16, out_channels=16, kernel_size=5, padding=0),  # -> [bs, 16, 4, 4]
            new_block(in_channels=16, out_channels=16, kernel_size=4, padding=0),  # -> [bs, 16, 1, 1]
        )

        self.surrogate_linear = nn.Sequential(
            nn.Linear(in_features=16, out_features=1),  # -> [bs, 1, 1, 1]
        )
